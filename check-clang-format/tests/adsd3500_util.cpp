...
int Adsd3500::ResetAdsd3500() {
    ...
    // NOTE: Jetson uses named GPIO "PAC.00", not numeric gpio122
    // This matches the ADCAM SDK's PLATFORM_RESET_GPIO for NVIDIA
    int ret1 = system("echo 0 > /sys/class/gpio/PAC.00/value 2>/dev/null");
    usleep(1000000); // 1 second pulse
    int ret2 = system("echo 1 > /sys/class/gpio/PAC.00/value 2>/dev/null");
    usleep(7000000); // 7 seconds delay for reset completion

    if (ret1 != 0 || ret2 != 0) {
        printf("  Warning: GPIO reset via PAC.00 not available (this may be "
               "normal)\n");
        printf("  Note: Reset may be handled automatically by the driver\n");
    } else {
        printf("  Reset successful via GPIO PAC.00\n");
    }
...
int Adsd3500::ReadCCB(const char *filename) {
    ...
    data[11] = 0x13;
    data[15] = 1;
    v4l2_ctrl_set(fd, V4L2_CID_AD_DEV_CHIP_CONFIG, data);
    ...
    v4l2_ctrl_get(fd, V4L2_CID_AD_DEV_CHIP_CONFIG, data);
    ...
    for (uint32_t i = 1; i <= packetsNeeded; i++) {
        ...
        bool retval = v4l2_ctrl_set(fd, V4L2_CID_AD_DEV_CHIP_CONFIG, data);
        v4l2_ctrl_get(fd, V4L2_CID_AD_DEV_CHIP_CONFIG, data);
        if (retval == false) {
            return -1;
        }
...
int Adsd3500::GetImagerTypeAndCCB() {
    ...
    printf("Read imager_version=%d, ccb_version=%d\n", imager_version,
           ccb_version);
    ...
    case 3: {
        ccbVersion = CCBVersion::CCB_VERSION2;
        printf("CCB version is  CCB_VERSION2.\n");
        break;
    }
    case 4: {
        ccbVersion = CCBVersion::CCB_VERSION3;
        printf("CCB version is  CCB_VERSION3.\n");
        break;
    }
    default: {
        ccbVersion = CCBVersion::CCB_UNKNOWN;
        printf("Warning: Unknown CCB version %d (not in range 1-4)\n",
               ccb_version);
    }
    }
        ...
        if (inputFormat == "raw8") {
            ...
            if (depthBits == 12 && abBits == 16 && confBits == 0) {
                ...
                if (mode_num == 1 || mode_num == 0) {
                    ...
                    frame.pixelFormat =
                        V4L2_PIX_FMT_SRGGB8; // NVIDIA Jetson uses SRGGB
                } else {
                    std::cout
            ...
            if (depthBits == 12 && abBits == 12 && confBits == 0) {
                if (mode_num == 1) {
                    ...
                    frame.frameHeight = 4096;
                    frame.pixelFormat =
                        V4L2_PIX_FMT_SRGGB12; // NVIDIA Jetson uses SRGGB
                } else if (mode_num == 0) {
                    frame.frameWidth = 1024;
...
int Adsd3500::adsd3500_get_ini_key_value_pairs_from_ccb() {
    ...
    v4l2_ctrl_set(videoDevice.cameraSensorDeviceId, V4L2_CID_AD_DEV_CHIP_CONFIG,
                  data);
    data[0] = 0;
    data[1] = data[4];
    data[2] = data[5];
    v4l2_ctrl_set(videoDevice.cameraSensorDeviceId, V4L2_CID_AD_DEV_CHIP_CONFIG,
                  data);
    v4l2_ctrl_get(videoDevice.cameraSensorDeviceId, V4L2_CID_AD_DEV_CHIP_CONFIG,
                  data);
...
int32_t write_cmd(int fd, uint8_t *ptr, uint16_t len) {
    ...
    int32_t ret =
        v4l2_ctrl_set(fd, V4L2_CID_AD_DEV_CHIP_CONFIG, cmd_data) ? 0 : -2;
    usleep(110 * 1000);
    return ret;
}
...
int32_t read_cmd(int fd, uint8_t *ptr, uint16_t len, uint8_t *rcmd,
    ...
    if (v4l2_ctrl_set(fd, V4L2_CID_AD_DEV_CHIP_CONFIG, cmd_data) == false) {
        return -1;
    }
    usleep(110 * 1000);
