...
OVERFLOW_RECOVERY_FAILED = re.compile(r"context overflow recovery failed", re.IGNORECASE)
...
class PiRpcSession:
    ...
    def request(self, command: dict[str, Any], timeout: float = 10.0) -> dict[str, Any] | None:
...
def is_truly_idle(session: PiRpcSession) -> dict[str, Any] | None:
...
def run(argv: list[str], prompt: str, max_nudges: int, max_seconds: float, finalize_grace_seconds: float = DEFAULT_FINALIZE_GRACE_SECONDS) -> int:
    ...
    def check_idle_and_react() -> int | None:
